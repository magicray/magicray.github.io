...
universe_screen = ('3728407/biggest', 'mcak5aaeyz4ovi7yavenlnvr1jg6wjfv')
...
def main():
    ...
    prev_names = set([s['name'] for s in prev['data'] if s['rank'] <= len(prev['data'])//4])
    stock_names = set([s['name'] for s in stock_list if s['rank'] <= len(stock_list)//4])
